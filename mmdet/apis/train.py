...
from mmdet.datasets.pipelines.transforms import GridMask, TargetRandomErasing
...
class RandomErasingHook(Hook):

    def __init__(self, dataset):
        self.re_obj = None
        for obj in dataset[0].pipeline.transforms:
            if isinstance(obj, TargetRandomErasing):
                self.re_obj = obj

    def before_train_iter(self, runner):
        cur_iter = runner.iter
        max_iter = runner.max_iters
        if self.re_obj is not None:
            self.re_obj.set_prob(cur_iter, max_iter)
...
def _dist_train(model,
    ...
    # register hooks
    runner.register_hook(GridMaskHook(dataset))
    runner.register_hook(RandomErasingHook(dataset))
